# Hi, I'm Denny Mathew 👋

Senior Java Backend Engineer with 12+ years of software development experience across backend engineering, API integration, enterprise application development, and legacy modernization.

I focus on building scalable backend systems using **Java**, **Spring Boot**, **REST APIs**, **Microservices**, **SQL**, and **Distributed Systems**. My experience spans **banking, insurance, healthcare, and enterprise platforms**, with deep exposure to system design, production support, and DevOps workflows.

I’m currently focused on **Senior Java Backend Engineer**, **Backend Platform Engineer**, **API Modernization**, and **Integration Engineering** roles across **Remote** and **Europe-based** teams.

---

## 🚀 Core Focus Areas

- Java Backend Development
- Spring Boot Services
- REST APIs & API Design
- Microservices
- API Modernization
- SOAP to REST Migration
- SQL & Data Workflows
- Distributed Systems
- System Design
- CI/CD & DevOps Practices
- Docker & Kubernetes
- Azure & Azure DevOps
- Production Support & Release Coordination

---

## 🛠 Tech Stack

### Languages
Java, SQL, Python, Bash, PowerShell

### Backend & APIs
Spring Boot, REST APIs, Microservices, API Design, Integration Engineering, SOAP, XML, JSON

### Architecture & Systems
Distributed Systems, System Design, HLD, LLD, Scalable Backend Architecture

### Cloud & DevOps
Azure, Azure DevOps, CI/CD, Docker, Kubernetes, Linux

### Engineering Practices
Agile, Scrum, Scrum Ceremonies, Troubleshooting, Production Support, Release Management, Cross-Functional Collaboration

---

## 📌 Java Backend Project Portfolio

I’m building a focused set of Java backend repositories to demonstrate production-style backend engineering, API design, modernization, microservices, security, DevOps, and distributed systems thinking.

> All repository links below use the full GitHub URL format.

---

### 1) Spring Boot Order Service
**Repo name:** `springboot-order-service`

- Repository: https://github.com/DennyM55/Spring-Boot-Order-Service

A production-style Spring Boot REST API for managing orders, customers, products, and payments.

**This project highlights:**
- Spring Boot REST APIs
- Layered architecture
- DTOs and request/response models
- Validation
- Global exception handling
- Spring Data JPA
- MySQL or PostgreSQL integration
- Swagger / OpenAPI documentation
- Pagination and filtering
- Unit tests
- Integration tests
- Dockerfile
- GitHub Actions CI pipeline

---

### 2) Microservices E-Commerce Platform
**Repo name:** `microservices-ecommerce-platform`

- Repository: https://github.com/DennyM55/Microservices-E-Commerce-Platform

A Spring Boot microservices-based backend platform for e-commerce workflows.

**Planned services:**
- product-service
- order-service
- inventory-service
- notification-service
- api-gateway

**This project highlights:**
- Microservices architecture
- API Gateway
- Service-to-service communication
- Centralized configuration
- Service discovery
- Docker Compose setup
- Logging and tracing basics
- Resilience / fallback patterns
- Externalized config management

**Optional advanced features:**
- Kafka-based async communication
- Redis caching
- Circuit breaker patterns
- Prometheus / Grafana monitoring

---

### 3) SOAP to REST Modernization Demo
**Repo name:** `soap-to-rest-modernization-demo`

- Repository: https://github.com/DennyM55/SOAP-to-REST-Modernization-Demo

A Java-based modernization demo showing how legacy SOAP services can be transformed into RESTful APIs.

**This project highlights:**
- Legacy SOAP contract example
- REST replacement API
- XML to JSON transformation
- Mapping / adapter layer
- Backward compatibility strategy
- API versioning
- Migration-friendly integration design
- Request / response transformation examples
- Architecture comparison between SOAP and REST

**Optional advanced features:**
- Adapter pattern implementation
- Contract testing
- Transformation utility layer

---

### 4) Secure User Auth Service
**Repo name:** `secure-user-auth-service`

- Repository:https://github.com/DennyM55/Secure-User-Auth-Service

A Spring Boot authentication and authorization service with production-style backend security practices.

**This project highlights:**
- Spring Security
- JWT authentication
- Role-based authorization
- Registration and login APIs
- Password hashing
- Input validation
- Exception handling
- Audit logging
- Unit tests
- Integration tests

**Optional advanced features:**
- Refresh token flow
- OAuth2 basics
- Email verification
- Forgot password flow
- API rate limiting

---

### 5) Event-Driven Order Processing
**Repo name:** `event-driven-order-processing`

- Repository: https://github.com/DennyM55/Event-Driven-Order-Processing

An event-driven Java backend for asynchronous order workflows.

**This project highlights:**
- Spring Boot
- Kafka or RabbitMQ integration
- Event-driven communication
- Asynchronous order processing
- Inventory update events
- Payment events
- Notification events
- Retry handling
- Idempotency strategies
- Dead-letter handling
- Correlation IDs and structured logging

**Optional advanced features:**
- Redis support
- Outbox pattern explanation
- Event schema documentation
- Observability dashboard

---

### 6) Java DevOps & Observability Demo
**Repo name:** `java-devops-observability-demo`

- Repository: https://github.com/DennyM55/java-devops-observability-demo

A backend engineering demo focused on Dockerization, CI/CD automation, health monitoring, and deployment readiness.

**This project highlights:**
- Spring Boot service
- Dockerfile
- Docker Compose
- GitHub Actions pipeline
- Test/build automation
- Health checks
- Structured logging
- Metrics endpoint
- Deployment-ready project structure

**Optional advanced features:**
- Kubernetes manifests
- Helm chart
- Prometheus + Grafana integration
- Azure deployment notes

---

### 7) System Design Case Studies (Optional)
**Repo name:** `system-design-case-studies-java`

- Repository: https://github.com/DennyM55/system-design-case-studies-java

A collection of backend and distributed systems design notes.

**Planned topics:**
- Designing scalable order systems
- API gateway design
- Rate limiting strategies
- Cache vs database tradeoffs
- Monolith to microservices migration
- Messaging and consistency tradeoffs
- Scaling an auth service
- Distributed system reliability considerations

---

## ✅ Current Project

### [Video to Subtitle Generator](https://github.com/DennyM55/video-to-subtitle-generator)
A Java-based application that automates subtitle generation from video files using **FFmpeg** for audio extraction and **Whisper.cpp** for offline transcription.

**This project demonstrates:**
- Java-based workflow automation
- Multimedia processing integration
- External tool orchestration
- File-based backend processing
- Practical problem-solving through code

---

## 🎯 What These Repositories Are Designed To Showcase

Through these repositories, I am intentionally building a portfolio that demonstrates:

- Java backend fundamentals
- Spring Boot application design
- API development and documentation
- Microservices architecture
- Distributed systems thinking
- Security and authentication
- SOAP to REST modernization
- Docker and CI/CD workflows
- DevOps readiness
- Event-driven backend systems
- Clean engineering structure and production-style code

---

## 📍 Professional Links

- **Portfolio:** https://dennymathew.me/
- **LinkedIn:** https://www.linkedin.com/in/dennym-space/
- **GitHub:** https://github.com/DennyM55
- **Scaler Profile:** https://www.scaler.com/academy/profile/e78bd859848b/
- **YouTube:** https://www.youtube.com/@denny-mathew
